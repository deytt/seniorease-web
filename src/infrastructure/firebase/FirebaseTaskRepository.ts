...
/**
 * Normaliza o campo array `steps` do documento (ADR-023).
 * A sub-collection `tasks/{id}/steps` está bloqueada nas rules (`allow … if false`).
 */
function mapSteps(
  taskId: string,
  raw: unknown,
  ...
  return raw.map((item, index) => {
    ...
    return {
      id: typeof step.id === "string" && step.id ? step.id : `step_${index}`,
      taskId:
...
/**
 * Persistência de tarefas — contrato ADR-023:
 * `steps` é um array no documento `tasks/{taskId}` (não sub-collection).
 */
export class FirebaseTaskRepository implements ITaskRepository {
  ...
  async createTask(task: Omit<Task, "id" | "createdAt">): Promise<Task> {
    ...
    const steps = task.steps.map((step, index) => ({
      id: step.id && step.id !== "temp" ? step.id : `step_${index}`,
      taskId: docRef.id,
  ...
  async updateTask(taskId: string, task: Partial<Task>): Promise<Task> {
    ...
    if (task.steps !== undefined) {
      payload.steps = task.steps.map((step, index) => ({
  ...
  async completeTask(taskId: string): Promise<Task> {
    const docRef = doc(db, this.collectionName, taskId);
    const snapshot = await getDoc(docRef);
    const currentSteps = mapSteps(taskId, snapshot.data()?.["steps"]);
  ...
  private mapDocumentToTask(document: DocumentSnapshot): Task {
    ...
    return {
      ...
      steps: mapSteps(document.id, data["steps"]),
